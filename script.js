...
const techList = [
  ...
  {ckb:{t:'نۆرتێکس',d:'ئامێری پریمیم'},ar:{t:'نورتكس',d:'جهاز متقدم'},en:{t:'Northex',d:'Premium advanced device'}},
...
function initLightbox(){
  ...
  if(!lb||!lbImg)return;
  function openLightbox(src,alt){lbImg.src=src;lbImg.alt=alt||'';lb.classList.add('active');lb.setAttribute('aria-hidden','false');document.body.classList.add('lightbox-open');}
  function closeLightbox(){lb.classList.remove('active');lb.setAttribute('aria-hidden','true');document.body.classList.remove('lightbox-open');setTimeout(()=>{lbImg.src='';},300);}
  lbClose?.addEventListener('click',closeLightbox);
  lb.addEventListener('click',e=>{if(e.target===lb||e.target===lbImg&&e.target!==lbImg)closeLightbox();if(e.target===lb)closeLightbox();});
...
document.addEventListener('DOMContentLoaded',()=>{
  initLoading();initScrollProgress();initNavbar();initMobileMenu();
  initServices();initTech();initFAQ();initLanguage();
  initReveal();initReviewsCarousel();initSmoothScroll();initSearch();initLightbox();
});
/* =========================================================
   SINUHA SECOND HERO — ULTRA PREMIUM INTERACTIONS
   Cinematic reveal | Magnetic dots | Ambient light | Parallax
   ========================================================= */

(function() {
  const section = document.getElementById('second-hero');
  if (!section) return;

  const video = section.querySelector('video');
  const videoWrap = section.querySelector('.sh-video-wrap');
  const loader = section.querySelector('.sh-loader');
  const ambientLight = section.querySelector('.sh-ambient-light');
  const hotspots = section.querySelectorAll('.sh-hotspot');
  const dots = section.querySelectorAll('.sh-dot');

  // ===== 1. Intersection Observer: reveal section =====
  ...
  // ===== 3. Smooth mouse parallax on video =====
  ...
  section.addEventListener('mousemove', (e) => {
    ...
    if (ambientLight) {
      ...
      ambientLight.style.setProperty('--mouse-x', xPct + '%');
      ambientLight.style.setProperty('--mouse-y', yPct + '%');
    }
  });
  ...
  function lerpParallax() {
    ...
    const moveX = currentX * -14; // max 14px
    const moveY = currentY * -10;   // max 10px
    
    if (videoWrap) {
      videoWrap.style.transform = `translate(${moveX}px, ${moveY}px)`;
    }
  ...
  // ===== 4. Magnetic dot effect =====
  dots.forEach(dot => {
    const hotspot = dot.closest('.sh-hotspot');
    if (!hotspot) return;
    ...
    hotspot.addEventListener('mousemove', (e) => {
      ...
      if (distance < maxDist) {
        const pull = (1 - distance / maxDist) * 10;
        const core = dot.querySelector('.sh-dot-core');
        if (core) {
          core.style.transform = `translate(${distX * pull * 0.05}px, ${distY * pull * 0.05}px) scale(1.5)`;
        }
      }
    });
  ...
  // ===== 5. Pause video when off-screen (performance) =====
  ...
  // ===== 6. Cleanup on page hide =====
  document.addEventListener('visibilitychange', () => {
    if (document.hidden && video) {
      video.pause();
    } else if (!document.hidden && video && section.classList.contains('is-visible')) {
      video.play().catch(() => {});
    }
  });

  // ===== 7. Keyboard: ESC closes cards =====
...
function initScrollProgress(){
  ...
  const update=()=>{const max=document.documentElement.scrollHeight-window.innerHeight;fill.style.width=max>0?`${(window.scrollY/max)*100}%`:'0%';};
...
function initMobileMenu(){
  ...
  const close=()=>{toggle.classList.remove('active');menu.classList.remove('active');backdrop.classList.remove('active');toggle.setAttribute('aria-expanded','false');document.body.style.overflow='';};
  toggle.addEventListener('click',()=>{const open=!menu.classList.contains('active');toggle.classList.toggle('active',open);menu.classList.toggle('active',open);backdrop.classList.toggle('active',open);toggle.setAttribute('aria-expanded',String(open));document.body.style.overflow=open?'hidden':'';});
...
function observeNewItems(container){
  if(!('IntersectionObserver' in window)){container.querySelectorAll('.reveal-item').forEach(el=>el.classList.add('visible'));return;}
  const obs=new IntersectionObserver(entries=>{entries.forEach(e=>{if(e.isIntersecting){e.target.classList.add('visible');obs.unobserve(e.target);}});},{threshold:0.06,rootMargin:'0px 0px -30px 0px'});
  container.querySelectorAll('.reveal-item').forEach(el=>obs.observe(el));
}
...
function initFAQ(){
  ...
  let html='';
  for(let i=1;i<=5;i++){html+=`<div class="faq-item reveal-item"><button class="faq-question" aria-expanded="false"><span data-key="faqQ${i}"></span><span class="faq-icon">+</span></button><div class="faq-answer"><div><p data-key="faqA${i}"></p></div></div></div>`;}
  container.innerHTML=html;
  container.querySelectorAll('.faq-question').forEach(btn=>{
    btn.addEventListener('click',()=>{const item=btn.closest('.faq-item');const open=!item.classList.contains('open');container.querySelectorAll('.faq-item').forEach(el=>el.classList.remove('open'));if(open)item.classList.add('open');btn.setAttribute('aria-expanded',String(open));});
  });
}
...
function initSmoothScroll(){
  ...
  links.forEach(link=>{link.addEventListener('click',e=>{const sel=link.getAttribute('href');if(!sel||sel==='#')return;const target=document.querySelector(sel);if(!target)return;e.preventDefault();window.scrollTo({top:target.offsetTop-70,behavior:'smooth'});});});
  const updateActive=()=>{let current='home';sections.forEach(s=>{if(window.scrollY>=s.offsetTop-138)current=s.id;});navLinks.forEach(l=>{l.classList.toggle('active',l.getAttribute('href')===`#${current}`);});};
